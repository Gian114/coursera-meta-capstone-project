...
function BookingForm(props) {
    ...
    return (
        <form className="grid gap-5 max-w-52 mx-16 mt-8">
            ...
            <input type="date" value={props.dateTime.date} onChange={(e) => props.dispatch({type: 'date', value: e.target.value })} required 
                className='block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline' />
            <label className='block text-gray-700 text-sm font-bold mb-2'>Choose Time:</label>
            <select value={props.dateTime.time} onChange={(event) => props.dispatch({type: 'time', value: event.target.value})}
                className='block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline'>
